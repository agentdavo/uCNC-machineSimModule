...
int loadConfiguration(const char *filename, ucncAssembly **rootAssembly, ucncLight ***lights, int *lightCount) {
    ...
    // Use NULL as the third argument for mxmlLoadFile since we don't need a callback
    mxml_node_t *tree = mxmlLoadFile(NULL, file, MXML_NO_CALLBACK);
    fclose(file);
    if (!tree) {
    ...
    if (assembliesNode) {
        for (mxml_node_t *assemblyNode = mxmlFindElement(assembliesNode, assembliesNode, "assembly", NULL, NULL, MXML_DESCEND);
             assemblyNode;
             assemblyNode = mxmlFindElement(assemblyNode, assembliesNode, "assembly", NULL, NULL, MXML_DESCEND)) {
            ...
            char motionAxis = ' ';
            int invertMotion = 0;
            mxml_node_t *motionNode = mxmlFindElement(assemblyNode, assemblyNode, "motion", NULL, NULL, MXML_DESCEND);
            if (motionNode) {
            ...
            mxml_node_t *homeNode = mxmlFindElement(assemblyNode, assemblyNode, "home", NULL, NULL, MXML_DESCEND);
            if (homeNode) {
                mxml_node_t *homePositionNode = mxmlFindElement(homeNode, homeNode, "position", NULL, NULL, MXML_DESCEND);
                if (homePositionNode) {
                ...
                mxml_node_t *homeRotationNode = mxmlFindElement(homeNode, homeNode, "rotation", NULL, NULL, MXML_DESCEND);
                if (homeRotationNode) {
    ...
    if (actorsNode) {
        for (mxml_node_t *actorNode = mxmlFindElement(actorsNode, actorsNode, "actor", NULL, NULL, MXML_DESCEND);
             actorNode;
             actorNode = mxmlFindElement(actorNode, actorsNode, "actor", NULL, NULL, MXML_DESCEND)) {
    ...
    if (lightsNode) {
        for (mxml_node_t *lightNode = mxmlFindElement(lightsNode, lightsNode, "light", NULL, NULL, MXML_DESCEND);
             lightNode;
             lightNode = mxmlFindElement(lightNode, lightsNode, "light", NULL, NULL, MXML_DESCEND)) {
